...
def compute_ablation(cdr3_to_X, ag_to_X, hit, irr, cdr3_col, ag_col):
    ...
    for seed in range(5):
        ...
        # add values
        return (cdr3_col, seed, auc(fpr, tpr), accuracy_score(y_pred[0], y_pred['Pred%'].apply(round)))
...
for cdr3_col in cdr3_cols:
    for ag_col in ag_cols:
        ...
        cdr3_ablate = pd.Series(np.arange(30)).astype(int).astype(str) + cdr3_col
        ag_ablate = pd.Series(np.arange(15)).astype(int).astype(str) + ag_col
        # compute parameters
        params.append([cdr3_to_X_orig.loc[:, ~cdr3_to_X_orig.columns.isin(cdr3_ablate)],
                       ag_to_X_orig.loc[:, ~ag_to_X_orig.columns.isin(ag_ablate)],
                       hit, irr, cdr3_col, ag_col])

import multiprocessing as mp
with mp.Pool(3) as pool:
    for el in tqdm(pool.imap_unordered(compute_ablation, params), total=len(params)):
        print(f'completed {el}')
        els.append(el)
        
import pickle as pkl
with open('tmp.csv.pkl', 'wb') as f:
    pkl.dump(els, f)
